...
- `/竞技场模型名称 GPT`：把 GPT 画图名称集中在一张表里，区分**可选 / 上游未开放 / 当前配置未放行**；
...
- 名称查询是独立的只读对照表：direct 模式会显示上游登记但未开放的画图名称，并按上游明确给出的
  `displayName` 合并名称对照；不猜测灰测模型身份，也不把未开放条目加入切换列表。
  旧 Bridge 后端仅返回可选模型时会注明查询范围。名称表不使用切换编号，也不发送出图请求
